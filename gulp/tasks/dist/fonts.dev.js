...
var otfToTtf = function otfToTtf() {
  return app.gulp
    .src(''.concat(app.path.srcFolder, '/fonts/*.otf'), {})
    .pipe(
      app.plugins.plumber(
...
var ttfToWoff = function ttfToWoff() {
  return app.gulp
    .src(''.concat(app.path.srcFolder, '/fonts/*.ttf'), {})
    .pipe(
      app.plugins.plumber(
        app.plugins.notify.onError({
          title: 'FONTS',
    ...
    .pipe(app.gulp.dest(''.concat(app.path.build.fonts)))
    .pipe(app.gulp.src(''.concat(app.path.srcFolder, '/fonts/*.ttf')))
    .pipe((0, _gulpTtf2woff['default'])())
    .pipe(app.gulp.dest(''.concat(app.path.build.fonts)));
};
        ...
        for (var i = 0; i < fontsFiles.length; i++) {
          ...
          if (newFileOnly !== fontFileName) {
            ...
            _fs['default'].appendFile(
              ...
              '@font-face{\n\tfont-family: '
                .concat(fontName, ';\n\tfont-display: swap;\n\tsrc: url("../fonts/')
                .concat(fontFileName, '.woff2") format("woff2"), url("../fonts/')
                .concat(fontFileName, '.woff") format("woff");\n\tfont-weight: ')
                .concat(fontWeight, ';\n\tfont-style: normal;\n}\r\n'),
              cb,
            );
          }
